...
zero ≡ℕ? suc n = no z≢ℕs
suc m ≡ℕ? zero = no s≢ℕz
suc m ≡ℕ? suc n with m ≡ℕ? n
...                | yes refl = yes refl
...                | no  m≢ℕn = no (s≢ℕs m≢ℕn)
...
yes x ⇔-dec no ¬y = no (λ x⇔y → ¬y (to x⇔y x))    where open _⇔_
no ¬x ⇔-dec yes y = no (λ x⇔y → ¬x (from x⇔y y))  where open _⇔_
